  ...
  implements physics.GeometryInterface
{
  ...
  public DoublePair quadraticSolution(double a, double b, double c) {
    if (a == 0.0) {
      if (b == 0.0) {
	return physics.Geometry.DOUBLE_PAIR_NAN;
      } else {
  ...
  public physics.Vect perpendicularPoint(physics.LineSegment line,
    ...
    return new physics.Vect(pptx, ppty);
  }
  ...
  public double timeUntilWallCollision(physics.LineSegment line,
	...
	if (velocity.dot(new physics.Vect(a - impactX, b - impactY)) >= 0) {
	  return Double.POSITIVE_INFINITY;
	} else {
	  return 0;
	}
                                  ...
                                  physics.Vect velocity)
  {
    ...
    physics.Angle theta = line.angle();     // angle of wall to horizontal
    physics.Angle alpha = velocity.angle(); // angle of velocity to horizontal

    // The angle of incidence is alpha-theta, the angle of reflection
    // is therefor theta = (alpha-theta) = 2 theta - alpha.
    physics.Angle phi = theta.plus(theta).minus(alpha);

    return new physics.Vect(phi, length);
  }
  ...
  public double timeUntilCircleCollision(physics.Circle circle,
                                         physics.Circle ball,
                                         physics.Vect velocity) {
    physics.Vect xy = circle.getCenter(),
         ab = ball.getCenter();
    double x       = xy.x(),
           y       = xy.y(),
    ...
    if (Double.isNaN(ans)) {
      ...
      physics.Vect impactDirection = ab.minus(xy);
                                    ...
                                    physics.Vect velocity)
  {
    ...
    physics.Vect incidence = circle.minus(ball);

    // It is as if the moving ball hits a wall perpendicular to the
    // angle of incidence.
    physics.Angle theta = incidence.angle().plus(physics.Angle.RAD_PI_OVER_TWO);

    // outgoing angle
    physics.Angle phi = theta.plus(theta).minus(velocity.angle());
  ...
  public physics.Vect rotateAround(physics.Vect point, physics.Vect cor, physics.Angle a) {
    physics.Vect original = point.minus(cor);
    physics.Angle newAng = original.angle().plus(a);
    physics.Vect after = new physics.Vect(newAng, original.length());
  ...
  public physics.LineSegment rotateAround(physics.LineSegment line, physics.Vect cor, physics.Angle a) {
    return new physics.LineSegment(rotateAround(line.p1(), cor, a),
  ...
  public physics.Circle rotateAround(physics.Circle circle, physics.Vect cor, physics.Angle a) {
    return new physics.Circle(rotateAround(circle.getCenter(), cor, a),
		      circle.getRadius());
  }
   ...
   *           from 0 to 4pi.
   **/
  private void restrictIntervalByAngle(physics.IntervalList intervals,
                                       double alpha,
                                       double beta,
                                       double omega,
                                       double phi,
   ...
   **/
  protected physics.IntervalList restrictSearchInterval(physics.IntervalList intervals,
                                                        double inner_radius,
                                                        double outer_radius,
                                                        double phi_1,
                                                        double phi_2,
    ...
    if (velocity.length() == 0) {
      physics.Vect ballCenter = ball.getCenter();
	...
	return new physics.IntervalList();
      }
    }
    ...
    DoublePair dp =
      timeUntilCircleCollision(new physics.Circle(center,
    ...
    physics.Vect ballVelocityNormal = velocity.rotateBy(physics.Angle.RAD_PI_OVER_TWO);
    ...
    physics.Vect farRailStartPoint = ball.getCenter().plus(ballVelocityNormal);
    physics.Vect nearRailStartPoint = ball.getCenter().minus(ballVelocityNormal);
    ...
    DoublePair farOuterIntersections =
      timeUntilCircleCollision(new physics.Circle(center, outer_radius),
			       farRailStartPoint,
			       velocity);
    ...
    DoublePair farInnerIntersections =
      timeUntilCircleCollision(new physics.Circle(center, inner_radius),
			       farRailStartPoint,
			       velocity);
    // intersection of the near side of the balls locus of travel with
    // the inner edge of the doughnut swept by the line
	...
	throw new IllegalArgumentException();
      ...
      physics.IntervalList intervals2 = new physics.IntervalList(intervals);
   ...
   **/
  private double searchForCollision(physics.Newton.Function distanceFunction,
                                    physics.IntervalList intervals) {
    Iterator iter = intervals.iterator();
    while (iter.hasNext()) {
      physics.IntervalList.Interval interval = (physics.IntervalList.Interval) iter.next();

      if (interval.start() < 0) {
	if (interval.end() < 0) {
	  continue;
	}
	interval = new physics.IntervalList.Interval(0, interval.end());
      }
	...
	continue;
      ...
      double collisionTime = physics.Newton.findRoot(distanceFunction,
					     interval.start(),
					     interval.end(), 
					     t_step);
      
      if (collisionTime >= 0) {
	physics.Newton.Result result = distanceFunction.evaluate(collisionTime);
	if (result.f_prime < 0) {
	  return collisionTime;
	} else {
	  return Double.POSITIVE_INFINITY;
                                               ...
                                               physics.Vect velocity)
  {
    ...
    class RotatingWallDistance implements physics.Newton.Function {
      public physics.Newton.Result evaluate(double t) {
	...
	if (!((0.0 <= fraction) && (fraction <= 1.0))) {
	  return new physics.Newton.Result(Double.NaN, Double.NaN);
	}
	...
	return new physics.Newton.Result(f, f_prime);
      }
    }
    ...
    physics.Newton.Function function = new RotatingWallDistance();
    physics.Newton.Result initialDistance = function.evaluate(0);
    ...
    physics.Vect distToCenter = perpendicularPoint(line, center);
    ...
    physics.IntervalList intervals = new physics.IntervalList(0, maximumForesight);
                                          ...
                                          double reflectionCoeff)
  {
    ...
    physics.Vect p1 = line.p1().minus(center);
    physics.Vect p2 = line.p2().minus(center);
    physics.Vect ballCenter = ball.getCenter().minus(center);
    line = new physics.LineSegment(p1, p2);

    // calculate the point at which the ball and wall collide
    physics.Vect perpPt = perpendicularPoint(line, ballCenter);
    ...
    physics.Vect myVel = perpPt.times(angularVelocity).rotateBy(physics.Angle.RAD_PI_OVER_TWO);

    // translate into reference frame of the moving wall
    physics.Vect relativeV = velocity.minus(myVel);

    physics.Vect reflectV = reflectWall(line, relativeV, reflectionCoeff);

    // now translate back into the absolute reference frame.
    physics.Vect absoluteV = myVel.plus(reflectV);
                                                 ...
                                                 physics.Vect velocity)
  {
    ...
    class RotatingCircleDistance implements physics.Newton.Function {
      public physics.Newton.Result evaluate(double t) {
    ...
    physics.Newton.Function function = new RotatingCircleDistance();
    physics.Newton.Result initialDistance = function.evaluate(0);
                                            ...
                                            double reflectionCoeff)
  {
    ...
    // translate everything so that center is the origin
    physics.Vect circleCenter = circle.getCenter().minus(center);
    physics.Vect ballCenter = ball.getCenter().minus(center);

    // calculate the point at which the ball and circle collide
    double ratio = radius1 / (radius1 + radius2);
    physics.Vect diff = circleCenter.minus(ballCenter);
    physics.Vect collidePt = circleCenter.plus(diff.times(ratio));
    ...
    // rotation by PI radians.
    physics.Vect myVel =
	collidePt.times(angularVelocity).rotateBy(physics.Angle.RAD_PI_OVER_TWO);
    // translate into reference frame of the moving circle
    physics.Vect relativeV = velocity.minus(myVel);

    physics.Vect reflectV = reflectCircle(circleCenter, ballCenter, relativeV,
				  reflectionCoeff);
  ...
  public double timeUntilBallBallCollision(physics.Circle ball1,
                                           ...
                                           physics.Vect vel2) {
    physics.Vect pos1             = ball1.getCenter();
    physics.Vect pos2             = ball2.getCenter();
    ...
    if (initGap2 <= 0.0) {
      ...
      physics.Vect velDelta = new physics.Vect(velXDelta, velYDelta);
      // 1's position if 2 were at the origin
      physics.Vect initPosDelta = new physics.Vect(initPosXDelta, initPosYDelta);
  ...
  public VectPair reflectBalls(physics.Vect center1,
    ...
    physics.Vect tHat = center1.minus(center2).unitSize();
